...
const InitAnimations = () => {
  useEffect(() => {
    AOS.init({
      duration: 250, // Even faster global animation duration for AOS
    });
  }, []);
        ...
        if (aipWrap && aipInner) {
          ...
          gsap.fromTo(
            ...
            },
            {
              scale: 1,
              opacity: 1,
              duration: 0.35, // Even faster (was 0.7)
              scrollTrigger: {
                trigger: aipWrap,
                start: "top 99%",
                markers: false,
    ...
    if (window.innerWidth >= 992) {
      ...
      titleAnims.forEach((titleAnim) => {
        ...
        charElements.forEach((char, index) => {
          ...
          tl2.from(char, {
            duration: 0.18, // Even faster (was 0.4)
            x: 70,
            delay: index * 0.015, // Reduce delay between chars
            autoAlpha: 0,
          });
        });
      });
